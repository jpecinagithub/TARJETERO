...
const CardItem: React.FC<CardItemProps> = ({ item }) => {
  ...
  return (
    <div className="group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col">
      <div className={`h-32 flex items-center justify-center ${isPdf ? 'bg-red-50' : 'bg-blue-50'}`}>
        {isPdf ? (
          <FileText className="w-12 h-12 text-red-500 opacity-80 group-hover:scale-110 transition-transform" />
        ) : (
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        {item.tags && item.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-6">
            {item.tags.map((tag) => (
              <span key={tag} className="inline-flex items-center gap-1 text-[10px] text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
